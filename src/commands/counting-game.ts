...
import { ICountingGameData } from '../schemas/index';

export default {
    data: new SlashCommandBuilder()
        .setName('counting-game')
        .setDescription('Counting Game Setup/Tuning Command.')
        .setNameLocalizations({ 'tr': 'sayı-sayma-oyunu' })
        .setDescriptionLocalizations({ 'tr': 'Sayı Sayma Oyun Kurulumu / Ayarlama Komudu.' })
    ...
    async execute(client: Client, interaction: CommandInteraction, options: CommandInteractionOptionResolver) {
        ...
        var dataObject = {} as ICountingGameData;
        ...
        await db.set(`countingGame_${interaction.guildId}`, dataObject);
