...
import { IBoomGameData } from '../schemas/index';

export default {
    data: new SlashCommandBuilder()
        .setName('boom-game')
        .setDescription('Boom Game Setup/Tuning Command.')
        .setNameLocalizations({ 'tr': 'bom-oyunu' })
        .setDescriptionLocalizations({ 'tr': 'Bom Oyun Kurulumu / Ayarlama Komudu.' })
    ...
    async execute(client: Client, interaction: CommandInteraction, options: CommandInteractionOptionResolver) {
        ...
        var dataObject = {} as IBoomGameData;
        ...
        await db.set(`boomGame_${interaction.guildId}`, dataObject);
